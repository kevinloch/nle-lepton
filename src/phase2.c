...
double solveNLEforMasses(nle_config_t *nle_config, nle_state_t *nle_state) {
  ...
  double term1_massratio_sm1, term2_massratio_sm1, term3_massratio_sm1;
  double term1_massratio_sm2, term2_massratio_sm2, term3_massratio_sm2;
  double term1_massratio_sm3, term2_massratio_sm3, term3_massratio_sm3;
  double mp=0;
  double worst_test;
  double rangefactor;
  ...
  if (nle_state->all_uses.float_alpha_em == 0) {
    alpha_center=(double)nle_config->ref_alpha;
    alpha_range=(double)nle_config->ref_alpha_error;
  }
  ...
  for (alpha_seq=(!nle_state->all_uses.alpha_em || nle_state->all_uses.float_alpha_em || nle_config->phase2_ignore_small_rel_unc); alpha_seq <= 1; alpha_seq++) {
    if (nle_state->all_uses.float_alpha_em == 0) {
      if (nle_config->phase2_ignore_small_rel_unc == 1) {
        alpha=(double)nle_config->ref_alpha;
      } else {
        if (alpha_seq == 0) {
                    ...
                    for (mh0_seq=(!nle_state->all_uses.mh0 || nle_state->all_uses.float_mh0); mh0_seq <= 1; mh0_seq++) {
                      ...
                      for (muser_seq=(!nle_state->all_uses.m_user || nle_state->all_uses.float_muser || (nle_config->smrfactor_mass_user_error == 0)); muser_seq <= 1; muser_seq++) {
                        ...
                        if (nle_state->all_uses.float_alpha_em == 1) {
                          alpha_last=(double)nle_config->ref_alpha;
                          alpha_center=(double)nle_config->ref_alpha;
                          alpha_range=(double)nle_config->ref_alpha * 0.1;
                        }
                        ...
                        for (samples=0; (precision_last > precision_target); samples++) {
                          ...
                          if (nle_state->all_uses.float_alpha_em == 1) {
                            ...
                            i=0;
                            while ((alpha < nle_config->ref_alpha * 0.9) || (alpha > (nle_config->ref_alpha * 1.1))) { // sanity check on alpha to help solve correct root and speed convergance
                          ...
                          if (nle_state->all_uses.float_sm1 == 1) {
                            ...
                            i=0;
                            while ((sm1 < nle_config->ref_sm1 * 0.9) || (sm1 > (nle_config->ref_sm1 * 1.1))) { // sanity check on sm1 mass to help solve correct root and speed convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_sm2 == 1) {
                            ...
                            i=0;
                            while ((sm2 < nle_config->ref_sm2 * 0.9) || (sm2 > (nle_config->ref_sm2 * 1.1))) { // sanity check on sm2 mass to help solve correct root and speed convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_v == 1) {
                            ...
                            i=0;
                            while ((v < (nle_config->ref_v * 0.9)) || (v > (nle_config->ref_v * 1.1))) { // sanity check to help convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_mz == 1) {
                            ...
                            i=0;
                            while ((mz < nle_config->ref_mz * 0.9) || (mz > (nle_config->ref_mz * 1.1))) { // sanity check on mz mass to help solve correct root and speed convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_G == 1) {
                            ...
                            i=0;
                            while ((G < (nle_config->ref_G * 0.9)) || (G > (nle_config->ref_G * 1.1))) {  // sanity check to help convergance
                              if (i > 50) {  // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_sm3 == 1) {
                            ...
                            i=0;
                            while ((sm3 < nle_config->ref_sm3 * 0.9) || (sm3 > (nle_config->ref_sm3 * 1.1))) { // sanity check on sm3 mass to help solve correct root and speed convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_mw == 1) {
                            ...
                            i=0;
                            while ((mw < nle_config->ref_mw * 0.9) || (mw > (nle_config->ref_mw * 1.1)) || (mw >= mz)) { // sanity check on mw mass to help solve correct root and speed convergance and prevent mw >= mz
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_sin2w == 1) {  
                            ...
                            i=0;
                            while ((sin2w < nle_config->ref_sin2w * 0.9) || (sin2w > (nle_config->ref_sin2w * 1.1))) { // sanity check on sin2w to help solve correct root and speed convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_mh0 == 1) {
                            ...
                            i=0;
                            while ((mh0 < nle_config->ref_mh0 * 0.9) || (mh0 > (nle_config->ref_mh0 * 1.1))) { // sanity check on mh0 mass to help solve correct root and speed convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          if (nle_state->all_uses.float_muser == 1) {
                            ...
                            i=0;
                            while ((muser < nle_config->smrfactor_mass_user * 0.9) || (muser > (nle_config->smrfactor_mass_user * 1.1))) { // sanity check on m_user mass to help solve correct root and speed convergance
                              if (i > 50) { // safety valve in case search gets out of bounds
#ifdef DEBUG20
                                clock_gettime(CLOCK_REALTIME, &end_time);
                                elapsed_time=((double)(end_time.tv_sec - 1500000000) + ((double)end_time.tv_nsec / 1.0E9)) - ((double)(start_time2.tv_sec - 1500000000) + ((double)start_time2.tv_nsec) / 1.0E9);
                          ...
                          term1_massratio_sm1=sm1 / term1_reference_mass;
                          term1_massratio_sm2=sm2 / term1_reference_mass;
                          term1_massratio_sm3=sm3 / term1_reference_mass;
                          term2_massratio_sm1=sm1 / term2_reference_mass;
                          term2_massratio_sm2=sm2 / term2_reference_mass;
                          term2_massratio_sm3=sm3 / term2_reference_mass;
                          term3_massratio_sm1=sm1 / term3_reference_mass;
                          term3_massratio_sm2=sm2 / term3_reference_mass;
                          term3_massratio_sm3=sm3 / term3_reference_mass;
                        ...
                        alpha_out_diff=alpha_out_c - nle_config->ref_alpha;
                        alpha_out_reldiff=alpha_out_diff / nle_config->ref_alpha;
  ...
  valid_result=1;
  if ((nle_state->all_uses.float_alpha_em == 1) && ((fabs(alpha_out_reldiff) / (phase2_results_window * nle_config->ref_alpha_relerror)) > 1.0)) {
    valid_result=0;
  }
  ...
  if ((valid_result == 1) || (nle_config->phase2_results_always == 1)) {
    ...
    if ((nle_state->term1.current_match->outfactor_user_exp_up != 0) || (nle_state->term2.current_match->outfactor_user_exp_up != 0) || (nle_state->term3.current_match->outfactor_user_exp_up != 0)) {
      sprintf(user_out_str, "uout=%.9e,", nle_config->outfactor_user);
    } else {
      sprintf(user_out_str, "                     ");
    }
