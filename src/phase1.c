...
int solveNLEforCoefficients(nle_config_t *nle_config, nle_state_t *nle_state) {
  ...
  if (nle_config->smrfactor_1minus_enable == 1) {
    ...
    two_term_precision=1.0E-5;       // check two_term_test after reaching this precision.  Disabling current ordering if not close enough to an integer
  ...
  if (nle_config->smrfactor_1minus_enable == 1) {
    ...
    if (nle_state->term1.smrfactor_mass == 0) {
      reference_mass=nle_state->input_sample_mp;
      sprintf(mass_str, "mP   ");
    } else if (nle_state->term1.smrfactor_mass == 1) {
      reference_mass=nle_state->input_sample_v;
      sprintf(mass_str, "v    ");
    } else if (nle_state->term1.smrfactor_mass == 2) {
      reference_mass=nle_state->input_sample_mz;
      sprintf(mass_str, "mz   ");
    } else if (nle_state->term1.smrfactor_mass == 3) {
      reference_mass=nle_state->input_sample_mw;
      sprintf(mass_str, "mw   ");
    } else if (nle_state->term1.smrfactor_mass == 4) {
      reference_mass=nle_state->input_sample_mh0;
      sprintf(mass_str, "mh0  ");
    } else if (nle_state->term1.smrfactor_mass == 5) {
      reference_mass=nle_state->input_sample_muser;
  ...
  if (nle_state->term1.smrfactor_1minus == 1) {
    ...
    term1_mass_sm3=powl(((long double)nle_state->input_sample_sm3 / (long double)reference_mass), (1.0 / (long double)nle_state->term1.exp_inv));
    term2_mass_sm1=powl(((long double)nle_state->input_sample_sm1 / (long double)reference_mass), (1.0 / (long double)nle_state->term2.exp_inv));
        ...
        active_ordering_count++;
          ...
          if ((samples > 1) && ((samples % slowcheckpoint) == 0)) {
            ...
            if ((samples % 10000000) == 0) { // rate limit periodic debug prints
              clock_gettime(CLOCK_REALTIME, &endtime);
              elapsed_time=((double)(endtime.tv_sec - 1500000000) + ((double)endtime.tv_nsec / 1.0E9)) - ((double)(starttime.tv_sec - 1500000000) + ((double)starttime.tv_nsec) / 1.0E9);
              printf("debug, exponents: %s, samples: %10lld, time: %6.4fs, ordering: %d, progress: %6d, best_precision_last: %.9Le, best_ordering: %d, precision_last: %.9Le, range_factor: %.9Le, sm1_test: %.9Le, sm2_test: %.9Le, sm3_test: %.9Le, c1: %.9Le, c2: %.9Le, c3: %.9Le, c1range: %.9Le, c2range: %.9Le, c3range: %.9Le\n", nle_state->exponents_str, samples, elapsed_time, ordering, progress[ordering], best_precision_last, best_ordering, precision_last[ordering], range_factor, sm1_test, sm2_test, sm3_test, c1_center[ordering], c2_center[ordering], c3_center[ordering], c1_range[ordering], c2_range[ordering], c3_range[ordering]);
              fflush(stdout);
            }
#endif
